...
uniform vec4 uScrollUV; // uCutoff.xy = U,V speed
...
void main() {
	...
	vec2 scroll_uv = vTexCoord0 + uScrollUV.xy * uClock.x;
	scroll_uv = vec2(mod(scroll_uv.x, 1.0), mod(scroll_uv.y, 1.0));
	vec4 base_opacity = texture2D(uBaseOpacityMap, scroll_uv);
