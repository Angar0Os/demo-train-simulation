...
uniform vec4 uNormal; // .x = min distance (normal is unchanged), .y = min distance (normal is blended), .z = min blend factor, .w = max blend factor
...
// Optional secondary occlusion, always needing a second set of UV (UV1)
	...
	// blend normal in the distance
	vec3 up_N = vec3(0.0, 1.0, 0.0);
	float blend_normal_dist = clamp(map(view.z, uNormal.x, uNormal.y, uNormal.z, uNormal.w), uNormal.z, uNormal.w);
	N = mix(N, up_N, blend_normal_dist);
	occ_rough_metal.x *= mix(1.0, 0.8, blend_normal_dist);
	...
	// gl_FragColor = vec4(vec3(blend_normal_dist, blend_normal_dist, blend_normal_dist), opacity);
