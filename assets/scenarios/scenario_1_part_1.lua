...
if CONFIGURATION == "Etat actuel" then
    api.LoadScene('scenario_1_part_1.scn', 'scene')
...
elseif CONFIGURATION == "Piste 3 - Ascenseur" then
    api.LoadWaypoints('../sncf-reims-2025/scenarios/scenario_1_part_1b.waypoints')
    api.LoadScene('scenario_1_part_1b.scn', 'scene')
...
api.LoadScene('envs/tunnel_v_lights.scn', 'tunnel_lighting')

local sgn_reperes_voies_quai_anim = api.PlayNodeAnimation("sgn_reperes_voies_quai", "scenario_1_depart_state_1")
...
voice_over = api.PlaySound('sounds/voice_over/s1_voice_01.ogg', 1.0)
api.WaitSource(voice_over)

api.GoToNode("interaction_point_timetable")
...
if CONFIGURATION == "Piste 3 - Ascenseur" then
    -- short path, right stairs
    api.GoToWaypoint(6)
    api.StopAnimation(sgn_reperes_voies_quai_anim)
    sgn_reperes_voies_quai_anim = api.PlayNodeAnimation("sgn_reperes_voies_quai", "scenario_1_depart_state_2")
    api.TeleportToWaypoint(6)
else
    -- original path, left stairs
    api.GoToWaypoint(6)
    api.TeleportToWaypoint(6)
...
api.GoToNode("interaction_point_train_v_1")

voice_over = api.PlaySound('sounds/voice_over/s1_voice_02.ogg', 1.0)
api.WaitSource(voice_over)

api.GoToNode("interaction_point_train_v_leaving")

-- END
api.ChangeScenario('scenario_001_part_002')
-- api.EndCapture()
